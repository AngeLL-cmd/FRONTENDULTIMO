...
// Variable para almacenar TensorFlow.js cuando esté disponible
let tfModule: any = null;
let tfLoadAttempted = false;

/**
 * Carga TensorFlow.js dinámicamente
 */
async function loadTensorFlow(): Promise<any> {
  if (tfModule) return tfModule;
  if (tfLoadAttempted) return null;
  
  tfLoadAttempted = true;
  
  try {
    // Usar una función que Vite no analice en tiempo de compilación
    const moduleName = '@tensorflow/tfjs';
    const dynamicImport = new Function('moduleName', 'return import(moduleName)');
    tfModule = await dynamicImport(moduleName);
    return tfModule;
  } catch (e) {
    console.warn('TensorFlow.js no está instalado. Ejecuta: npm install @tensorflow/tfjs');
    return null;
  }
}
